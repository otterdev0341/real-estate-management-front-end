...
import ReqCreatePaymentItemDto from "@/domain/payment/create/ReqCreatePaymentItemDto"
...
const initialState = {
  ...
  contactId: "",
  files: [] as FileUpload[],
  items: [] as ReqCreatePaymentItemDto[],
}

const CreatePaymentForm = ({ onSubmit, onCancel }: CreatePaymentFormProps) => {
  const { properties, loading: propertyLoading } = usePropertyContext()
  const { contacts, loading: contactLoading } = useContactContext()
  const { expenses, loading: expenseLoading } = useExpenseContext()
  ...
  const handleAddItem = () => {
    setItemInputs([...itemInputs, { expense: "", amount: "", price: "" }])
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Call PaymentService
      const result = await PaymentService.instance.createPayment(dto)
      if (isLeft(result)) {
        setSubmitError(result.value.message || "Failed to create payment")
  ...
  return (
    <>
      <h2 className="text-2xl font-bold text-foreground mb-2">Create Payment</h2>
      <p className="mb-6 text-muted-foreground">Fill in the details to create a new payment record</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label className="block text-sm font-medium text-foreground mb-2">Contact <span className="text-destructive">*</span></label>
          <CommonSelect
      ...
      {/* Note */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-foreground mb-2">Payment Note <span className="text-destructive">*</span></label>
        <textarea
          name="note"
          value={formData.note}
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 mt-6">
        ...
        >
          <span className="inline-flex items-center gap-2">
            <PlusIcon className="w-4 h-4" />
            Create payment (${grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
          </span>
        </button>
      </div>
    </>
