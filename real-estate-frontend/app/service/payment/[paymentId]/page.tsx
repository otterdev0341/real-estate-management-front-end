...
import ViewPaymentForm from "@/components/form/payment/ViewPaymentForm"
...
const fetchFiles = async (dto: BaseFetchFileRelatedDto) => {
  return PaymentService.instance.fetchPaymentFileRelated(dto)
}

const attachFile = async (dto: BaseAttachFileToTarget) => {
  return PaymentService.instance.attachFileToPayment(dto)
}

const removeFile = async (dto: BaseRemoveFileFromTarget) => {
  return PaymentService.instance.removeFileFromPayment(dto)
}
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <h1 className="text-2xl text-left font-bold sm:ml-6">Payment Details</h1>
        ...
        <div className="bg-card/60 backdrop-blur-xl border border-border rounded-xl p-4 sm:p-6 shadow-lg relative overflow-x-auto">
          {activeTab === "detail" && <ViewPaymentForm paymentId={paymentId} />}
