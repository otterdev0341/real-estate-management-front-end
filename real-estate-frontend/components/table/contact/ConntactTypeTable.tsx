...
const ConntactTypeTable = () => {
  ...
  const totalPages = Math.ceil(sortedContactTypes.length / rowsPerPage)
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full">
            ...
            <tbody>
              {paginatedContactTypes.length > 0 ? (
                paginatedContactTypes.map((type, index) => (
                  <tr
                    key={type.id}
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-muted-foreground">No contact types found</p>
          ...
          <div className="flex items-center justify-between px-6 py-4 bg-muted/30 backdrop-blur-sm border-t border-border">
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground">
                {startIndex + 1}-{Math.min(startIndex + rowsPerPage, sortedContactTypes.length)} of{" "}
                {sortedContactTypes.length}
              </span>
              <div className="flex items-center gap-1">
                <button
      ...
      <div className="lg:hidden space-y-4">
        {paginatedContactTypes.length > 0 ? (
          paginatedContactTypes.map((type) => (
            <div
              key={type.id}
          ...
          <div className="bg-card/60 backdrop-blur-xl border border-border rounded-xl p-8 text-center">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-foreground">No contact types found</p>
              <p className="text-sm text-muted-foreground">Try adjusting your search</p>
            </div>
          </div>
        )}
...
export default ConntactTypeTable