...
export default function UserDisplayCard({ isOpen, onClose, position }: UserDropdownProps) {
      ...
      >
        {loggingOut ? (
          <div className="flex flex-col items-center justify-center h-64">
            <svg className="animate-spin h-10 w-10 text-violet-700 mb-4" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
            </svg>
            <span className="text-lg font-semibold text-violet-700">Logging out...</span>
          </div>
        ) : (
          <>
            {/* Header */}
