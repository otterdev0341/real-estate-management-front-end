...
const CreatePropertyStatusForm = ({ onCancel, onSuccess }: CreatePropertyStatusFormProps) => {
  const [formData, setFormData] = useState<ReqCreatePropertyStatusDto>({ detail: "" })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await PropertyStatusService.instance.createNewPropertyStatus(formData)
      if (isLeft(result)) {
        setErrorLabel(result.value.message || "Failed to create property status")
        ...
        <div className="absolute inset-0 bg-white/95 backdrop-blur-sm rounded-lg flex items-center justify-center z-20">
          <div className="bg-white rounded-lg shadow-lg p-6 flex flex-col items-center space-y-3 animate-in fade-in zoom-in duration-300">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">Property status "{formData.detail}" has been created</p>
            </div>
          </div>
        </div>
      )}
      ...
      {isSubmitting && !isSuccess && (
        <div className="absolute inset-0 bg-white/50 backdrop-blur-sm rounded-lg flex items-center justify-center z-10">
          <div className="bg-white rounded-lg shadow-lg p-4 flex items-center space-x-3">
            ...
            <span className="text-sm font-medium text-gray-700">Creating property status...</span>
          </div>
        </div>
      )}
...
export default CreatePropertyStatusForm
